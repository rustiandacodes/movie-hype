...
const PopularMovie = () => {
  ...
  //   setpage(page + 1);
  //   console.log(page);
  //   getPopularMovies(page).then((result) => {
    ...
    <>
      <div className="bg-dark-bg">
        <div className="container mx-auto lg:p-8 p-5">
          <h1 className="text-lg font-bold mb-5">Popular This Week</h1>
          <div className="flex justify-between items-center flex-wrap gap-1">
            {movies.length > 0 &&
              movies.map((movie, i) => (
                <div className="relative w-[48%] md:w-1/6 max-h-96 overflow-hidden my-3 lg:mb-5 rounded-lg h-full hover:scale-110 hover:duration-300 cursor-pointer" key={i}>
                  <img className="h-fit " src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`} alt="poster" />
                  <div className="absolute bottom-0 left-0 right-0 top-0 bg-gradient-to-t from-black">
                    <div className="absolute bottom-0 left-0 right-0 lg:px-3 lg:py-5 p-2">
                      <h2 className="text-sm font-bold shadow-sm lg:text-lg truncate">{movie.title}</h2>
